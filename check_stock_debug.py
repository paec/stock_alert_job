...
print(f"Close: {df['Close']}")  # 只印 Close 欄位
    
    
# 抓取最近 1 分鐘的資料 (這會回傳包含最新一筆的時間)
df_now = ticker.history(period="1d", interval="1m")
print(f"history min:\n {df_now}")
print(f"Close: {df_now['Close']}")  # 只印 Close 欄位
if not df_now.empty:
...
print(f"Close: {df_daily['Close']}")  # 只印 Close 欄位
...
print(f"Close: {df_min['Close']}")  # 只印 Close 欄位
